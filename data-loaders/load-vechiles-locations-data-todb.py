...
def connect_to_rabbitmq(host=rabbitmq_host, queue_name=vechicle_location_queue_live):
...
# );
    cursor = conn.cursor()
    sql = """
        INSERT INTO vehicles_locations (vehicle_id, timestamp, latitude, longitude, speed, mileage, fuel_level, battery_level, temperature, engine_status, next_service_date, vehicle_alerts)
        VALUES (%s, %s, %s::numeric, %s::numeric, %s, %s, %s, %s, %s, %s, %s, %s)
    """
    cursor.execute(sql, (
        item_data['vehicle_id'],
        item_data['timestamp'],
        item_data['latitude'],
        item_data['longitude'],
        item_data['speed'],
        ...
        item_data['battery_level'],
        item_data['temperature'],
        item_data['engine_status'],
...
def main():
    ...
    rabbitmq_channel.basic_consume(queue=vechicle_location_queue_live, on_message_callback=callback, auto_ack=True)
...
# vechicle_location_queue_live = config.get('rabbitmq', 'vechicle_location_queue_live')
#
# def connect_to_rabbitmq(host='your_rabbitmq_host', queue_name=vechicle_location_queue_live):
#     """Establishes a connection to RabbitMQ."""
#
#     credentials = pika.PlainCredentials(rabbitmq_user, rabbitmq_password)
#     connection = pika.BlockingConnection(
...
#     cursor = conn.cursor()
#     sql = """
#         INSERT INTO vehicles_locations (vehicle_id, timestamp, gps_location, speed, mileage, fuel_level, temperature, engine_status, next_service_date, vehicle_alerts)
...
# rabbitmq_user = config.get('rabbitmq', 'rabbitmq_user')  # Handle potential errors
# rabbitmq_password = config.get('rabbitmq', 'rabbitmq_password')  # Handle potential errors
# vechicle_location_queue_live = config.get('rabbitmq', 'vechicle_location_queue_live')  # Handle potential errors
#
#
...
#
# if __name__ == '__main__':
#     consume_from_rabbitmq(vechicle_location_queue_live)
