...
def get_db_version():
    try:
        conn = psycopg2.connect(host=db_config.pg_host, password=db_config.pg_password, database=db_config.pg_database, user=db_config.pg_user, port=db_config.pg_port)
        cur = conn.cursor()
        ...
        with psycopg2.connect(host=db_config.pg_host, password=db_config.pg_password, database=db_config.pg_database, user=db_config.pg_user, port=db_config.pg_port) as conn1:
            with conn1.cursor() as curr:
                ...
                sql = """
                    ...
                    FROM vehicles
                    ORDER BY ABS(latitude - %s) ASC , ABS(longitude - %s) ASC
                    LIMIT 20;
                """
